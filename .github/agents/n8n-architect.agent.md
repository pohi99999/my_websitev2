...
- Generated context root hint: `f:\my_websitev2`. If this path exists, run workspace commands from there.
...
```bash
npx --yes n8nac@next env add <name> --base-url <url> --sync-folder workflows/<name>
...
```bash
npx --yes n8nac@next env add Local --managed-instance <id> --sync-folder workflows/local
