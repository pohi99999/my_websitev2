...
- **Netlify is discontinued — do not reintroduce it.** An old `pohankaestarsa.netlify.app` deployment was still live and auto-building from `main` long after the project moved to Vercel, silently mirroring the production site (fully crawlable, no noindex) and polluting the production GA4 property with real hits from its domain (caught via GA's "additional domains detected" tag diagnostic, fixed 2026-08-21 by gating `GoogleAnalytics` in `app/layout.tsx` to only load on the `www.pohankaestarsa.com`/`pohankaestarsa.com` request `Host`). The user has since torn down that Netlify site. `netlify.toml`, `vite.config.js`, and `deploy-build.sh` no longer exist in the repo — the only deploy path is Vercel + `next build`.
