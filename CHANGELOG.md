...
- **Missing `x-default` hreflang site-wide** — every page's `alternates.languages` listed `hu`/`en`/`de` but no `x-default`, even though `public/sitemap.xml` already declares one for every URL. Initially fixed only `app/layout.tsx`, then discovered Next.js metadata doesn't deep-merge `alternates` between layout and page — each page's own `generateMetadata`/`metadata` export fully overrides it, so the layout-level fix never reached any actual page. Added `'x-default'` (pointing at the `hu` URL) to all 18 files that declare their own `alternates.languages`: `app/layout.tsx`, `app/page.jsx`, `app/en/[[...slug]]/page.tsx`, `app/de/[[...slug]]/page.tsx`, and 14 static pages (`rolunk`, `fogaszati-lead-lista`, `weboldal-ai-kkv`, `blog`, `fogalomtar`, `portfolio` + its 5 case-study routes, `termekek` + `termekek/pohi-ai-pro`, `szolgaltatasok`). Four remaining pages (`blog/[slug]`, `portfolio/[id]`, `aszf`, `adatvedelmi-nyilatkozat`) have no `languages` map at all — only a bare `canonical` — a larger pre-existing gap (missing hreflang entirely, not just `x-default`) left as a follow-up.
...
- **Vercel project** (`pohi99999s-projects/my_websitev2`): latest production deployment healthy, zero runtime errors in the last 7 days. Deployment Protection (`ssoProtection`, scope `all_except_custom_domains`) is already correctly enabled, so all `*.vercel.app` aliases (`pohanka.vercel.app`, `mywebsitev2.vercel.app`, …) return 402 and can't be indexed as duplicate content next to `www.pohankaestarsa.com` — the same class of bug found and fixed on a sibling project (CARS SR99 KFT) the same day was already a non-issue here.
